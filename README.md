...
The chat panel is the actual complete `CodexChatEmbed` from `t3-code-ultralight-browser-fork`, not a local transcript approximation. It preserves separate Codex messages, image upload/paste/drop/zoom, per-thread drafts, `@` file mentions, `$` skills, `/` commands, Build/Plan, permission modes, anchored streaming, condensed tool work, thread and plan actions, rich Markdown controls, and the T3-derived composer while the connector supplies token-scoped transport paths.
